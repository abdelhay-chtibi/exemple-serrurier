          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center">
              ...
              <p className="text-xl text-primary-foreground/90 max-w-3xl mx-auto">
                Une urgence ? Un devis ? Nous sommes à votre écoute 24h/24 pour répondre à tous vos besoins en
                serrurerie.
              </p>
            </div>
          </div>
        ...
        <section className="bg-destructive text-destructive-foreground py-4">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-center space-x-2 text-center">
              ...
              <a href="tel:0612345678" className="font-bold underline hover:no-underline">
                06 12 34 56 78
              </a>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
              {contactInfo.map((info, index) => (
                <Card key={index} className={`text-center ${info.urgent ? "ring-2 ring-primary" : ""}`}>
                  <CardContent className="p-6">
                    <info.icon
                      className={`h-8 w-8 mx-auto mb-4 ${info.urgent ? "text-primary" : "text-muted-foreground"}`}
                    />
                    <h3 className="font-heading text-lg font-semibold mb-2">{info.title}</h3>
                    {info.details.map((detail, idx) => (
                      <p
                        key={idx}
                        className={`${idx === 0 && info.urgent ? "font-bold text-primary" : "text-muted-foreground"} ${idx === 0 ? "text-base" : "text-sm"}`}
                      >
                        {detail}
                      </p>
                    ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              {/* Contact Form */}
              <div>
                <h2 className="font-heading text-3xl font-bold text-foreground mb-6">Demande de Devis</h2>
                <p className="text-muted-foreground mb-8">
                  Remplissez ce formulaire pour recevoir un devis gratuit et personnalisé. Nous vous recontactons dans
              ...
              <div>
                <h2 className="font-heading text-3xl font-bold text-foreground mb-6">Notre Zone d'Intervention</h2>
                <div className="bg-muted rounded-lg p-6 mb-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm">Paris (tous arrondissements)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <MapPin className="h-4 w-4 text-primary" />
                ...
                <Card>
                  <CardContent className="p-6">
                    <h3 className="font-heading text-xl font-semibold mb-4">Informations Importantes</h3>
